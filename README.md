...
### Experiment V6: 3D 탐색형 음악 공간 — 실현 가능성 검증

> `python3 src/experiment_v6.py`

"인간이 3D 공간에서 축을 조절하며 음악을 탐색하는 시스템"의 실현 가능성을 4가지 테스트로 검증.

| 테스트 | 결과 | 근거 |
|--------|------|------|
| 축 해석 가능성 | **PASS** | PC1=밝기, PC2=강도, PC3=화성복잡도. 장르가 축 위에 일관되게 배치 |
| 탐색 일관성 | **PASS** | 축을 따라 이동 시 장르가 점진적으로 변화 (Classical→Rock→Electronic) |
| 지역 일관성 | **PARTIAL** | 근처 곡이 다른 장르일 수 있으나, 음향적으로는 유사 (의도된 결과) |
| 슬라이더 효과 | **PASS** | 각 축을 단독으로 조절 시 예측 가능한 변화 발생 |

**3축의 음악적 의미:**
- **Axis 1 (Brightness/Energy)**: 어두운/뮤트된 ↔ 밝은/선명한 소리
  - LOW: Classical(-0.88), Old-Time(-1.29), Folk(-0.47)
  - HIGH: Soul-RnB(+0.84), Hip-Hop(+0.58), Electronic(+0.55)
- **Axis 2 (Intensity/Loudness)**: 조용한/부드러운 ↔ 크고/강렬한
  - LOW: Hip-Hop(-0.48), Electronic(-0.33)
  - HIGH: Old-Time(+1.81), Folk(+0.54), Classical(+0.43)
- **Axis 3 (Harmonic Complexity)**: 단순한 ↔ 복잡한 화성 구조
  - LOW: Rock(-0.33), Spoken(-0.32)
  - HIGH: Classical(+0.92), Instrumental(+0.58), Jazz(+0.27)

**판정: FEASIBLE** — 3D 탐색형 음악 공간은 현실적으로 구현 가능하다.
...
- **균일 랜덤 샘플링**: 장르 편향 없이 전체 데이터에서 무작위 추출하여 큐브 전체에 고르게 시각화
- **실시간 k-NN**: 슬라이더 조작 시 즉시 주변 10곡을 검색하여 표시

**사용법:**
1. 🔴 **Brightness/Energy** 슬라이더: 어두운 ↔ 밝은 소리
2. 🟢 **Intensity/Loudness** 슬라이더: 조용한 ↔ 강렬한
3. 🔵 **Harmonic Complexity** 슬라이더: 단순 ↔ 복잡한 화성
4. 슬라이더를 조절하면 🎯 마커가 이동하고 주변 곡이 표시됨
5. 곡을 클릭하면 해당 위치로 점프
...
### V6 (`src/experiment_v6.py`)
- `output/v6_navigable_space.png` — 3D 탐색형 음악 공간 (인간 해석 가능한 축)
- `output/v6_2d_projections.png` — 2D 투영도 (축 쌍별 분포)
